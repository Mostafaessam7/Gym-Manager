...
public sealed class UpdateLeadCommandHandler(ILeadRepository leadRepository, IUnitOfWork unitOfWork, IBranchAccessGuard branchAccessGuard)
    : ICommandHandler<UpdateLeadCommand>
{
    public async Task<Result> Handle(UpdateLeadCommand command, CancellationToken cancellationToken)
    {
        ...
        lead.UpdateDetails(command.Name, command.Email, command.Phone, command.Source, command.Notes);

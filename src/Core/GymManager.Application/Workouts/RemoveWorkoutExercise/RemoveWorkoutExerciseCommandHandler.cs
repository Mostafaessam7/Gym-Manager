...
public sealed class RemoveWorkoutExerciseCommandHandler(
    ...
    : ICommandHandler<RemoveWorkoutExerciseCommand>
{
    public async Task<Result> Handle(RemoveWorkoutExerciseCommand command, CancellationToken cancellationToken)
    {
        ...
        var result = plan.RemoveExercise(command.ExerciseId);
        if (result.IsFailure)
            return result;

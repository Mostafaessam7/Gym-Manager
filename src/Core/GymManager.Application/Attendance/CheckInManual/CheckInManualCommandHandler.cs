...
public sealed class CheckInManualCommandHandler(
    ...
    : ICommandHandler<CheckInManualCommand, Result<AttendanceRecordResponse>>
{
    public async Task<Result<AttendanceRecordResponse>> Handle(CheckInManualCommand command, CancellationToken cancellationToken)
    {
        var member = await memberRepository.GetByIdAsync(command.MemberId, cancellationToken);
        if (member is null)
            return Result.Failure<AttendanceRecordResponse>(MemberErrors.NotFound);
        ...
        var record = AttendanceRecord.CheckIn(member.Id, member.BranchId, CheckInMethod.Manual, currentUserService.UserId);
